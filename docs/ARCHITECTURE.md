...
products         ─┤ written by product_scout          ← LEGACY, being deprecated
selectedProductId├ written by style_matcher           ← LEGACY, being deprecated
...
3. **Sequential suffix** — `furniture_planner` → `placement_validator` → `product_scout` → `style_matcher` → `pricing_estimator` → `navigation_planner`.
